...
public class PositionGenerate : MonoBehaviour
{
    ...
    public void PosGenerate(Vector3 pos1, Vector3 pos2, int width, List<Vector3> PointPos, int GetSelect)//計算點座標 (1)主線段點(2)右左兩個延伸點座標計算
    {
        //右左兩個延伸點座標矩陣
        ...
        if (GetLenPoint.Count > 2 && GetSelect == 1) diamandStyle(PointPos, width, Vec0);
    ...
    public void diamandStyle(List<Vector3> Point, int width, Vector3 Vec00)
    {
        tempPoint.Clear();
        GetUpdatePoint.Clear();
